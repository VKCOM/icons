/**
 * https://github.com/mapbox/jsxtreme-markdown/blob/main/packages/hast-util-to-jsx/index.js
 *
 * @import {Node, Root, Comment, Text, Element, Properties} from 'hast'
 */
...
import { convert as reactAttrConverter } from 'react-attr-converter';
...
function renderElement(ctx, node) {
  ...
  let renderedChildren = renderChildren(ctx, elementName === 'template' ? node.content : node);
  ...
  if (elementName === 'pre') {
    renderedChildren = renderedChildren.replace(/( {2,}|\n|\t)/g, (whitespace) => {
...
/**
 *
 * @param {Properties} hastProperties
...
function hastPropertiesToJsxProps(hastProperties, elementName) {
  return Object.keys(hastProperties).reduce((result, hastName) => {
    ...
    if (Array.isArray(value)) {
      const punc = info?.commaSeparated ? ', ' : ' ';
    ...
    if (info?.numeric && typeof value === 'string' && isNumberyString(value)) {
 ...
 * @returns {boolean}
 */
function isNumberyString(value) {
 ...
 * @returns {string}
 */
function renderComment(_ctx, node) {
  return `{/*${node.value}*/}`;
}
 ...
 * @returns {string}
 */
function renderText(_ctx, node, _index, parent) {
 ...
 * @returns {string}
 */
export function toJsx(node, options = {}) {
